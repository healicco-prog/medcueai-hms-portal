...
export const PreAuditReport = ({ user }: { user: User }) => {
  ...
  const fetchSavedReports = async () => {
    try {
      ...
      const res = await fetch('/api/consolidated-reports', {
  ...
  const handleDeleteReport = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`/api/consolidated-reports/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center print:hidden border-b border-slate-200 pb-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-800">Prescription Audit Report- Consolidated</h2>
          <p className="text-slate-500">View all consolidated Report- Monthly</p>
        </div>
          ...
          <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex justify-between items-start print:items-center">
              ...
              <div className="flex space-x-3 print:hidden">
                ...
                <SocialShareButton
                  shareText={`📊 Generated a WHO-compliant Prescription Audit Report for ${formatMonthName(reportData.month)} using MedCueAI.\n\n✅ Avg Score: ${reportData.institute.avgScore.toFixed(1)}/100\n✅ WHO Compliance: ${reportData.institute.avgCompliance.toFixed(1)}%\n✅ Prescriptions Audited: ${reportData.institute.count}\n\n🔗 https://medcueai.com\n\n#MedCueAI #PrescriptionAudit #WHOCompliance #HealthcareAI`}
                  emailSubject={`Prescription Audit Report — ${formatMonthName(reportData.month)}`}
                />
              </div>
            </div>
